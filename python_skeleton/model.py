...
class Model():
    ...
    def get_truncated_normal(self):
        ...
        a, b = (low - self.betting_mean) / self.betting_std, (high - self.betting_mean) / self.betting_std
        return truncnorm(a, b, loc=self.betting_mean, scale=self.betting_mean)
    
    # def get_beta_params(self, mean, std):
    #     # Calculate alpha and beta parameters based on mean and standard deviation
    #     var = std ** 2
    #     alpha = ((1 - mean) / var - 1 / mean) * mean ** 2
    #     beta_param = alpha * (1 / mean - 1)
    #     return alpha, beta_param

    # def get_beta_distribution(self, mean, std):
    #     # Get alpha and beta parameters
    #     alpha, beta_param = self.get_beta_params(mean, std)
    ...
    def generate_child(x, y):
        ...
        attributes = [attr_name for attr_name in vars(x) if attr_name != "betting_distribution"]
